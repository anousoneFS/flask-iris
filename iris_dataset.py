...
def load(path="dataset/iris.csv", split_train_test=None):
    ...
    if split_train_test:
        ...
        for i in classes:
            idx = np.where(Y == i)[
                0
            ]  # id where have setosa or versicolor (default 2 dimension)
            split = int(
                len(idx) * split_train_test
            )  # 50 * 0.5 = 25 //  25 + 25 + 25 = 75
            itrain = np.concatenate(
                (itrain, idx[:split])
            )  # Joining the two arrays along axis 0:
            itest = np.concatenate(
                (itest, idx[split:])
            )  # if Joining dimension should same
